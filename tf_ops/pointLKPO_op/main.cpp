...
void lkpoSelectLauncher(int b, int n, float radius, const float* xyz, int* idx_out);
class LKPOSelectOp : public OpKernel {
public:
    explicit LKPOSelectOp(OpKernelConstruction * context): OpKernel(context) {
        OP_REQUIRES_OK(context, context->GetAttr("radius", &radius_));
    }
    ...
    void Compute(OpKernelContext* context) override {
        ...
        auto idx_out_flat = idx_out_tensor->flat<int>();
        int* idx_out = &(idx_out_flat(0));
        lkpoSelectLauncher(b, n, radius_, xyz, idx_out);
    }
private:
    float radius_;
};
REGISTER_KERNEL_BUILDER(Name("LKPOSelect").Device(DEVICE_GPU), LKPOSelectOp);

void lkpoSelectFourLauncher(int b, int n, float radius, const float* xyz, int* idx_out);
class LKPOSelectFourOp : public OpKernel {
public:
    explicit LKPOSelectFourOp(OpKernelConstruction * context): OpKernel(context) {
        OP_REQUIRES_OK(context, context->GetAttr("radius", &radius_));
    }
    ...
    void Compute(OpKernelContext* context) override {
        ...
        auto idx_out_flat = idx_out_tensor->flat<int>();
        int* idx_out = &(idx_out_flat(0));
        lkpoSelectFourLauncher(b, n, radius_, xyz, idx_out);
    }
private:
    float radius_;
};
REGISTER_KERNEL_BUILDER(Name("LKPOSelectFour").Device(DEVICE_GPU), LKPOSelectFourOp);
