...
__global__ void lkpo_select(int b, int n,float radius, const float* xyz, int* idx_out) {
    ...
    for(int i = threadIdx.x; i < n;i += blockDim.x) {
        ...
        for(g=1;g<8;g++)
        {
             for(k=1;k<8-g;k++)
             {
                  if(temp_dist[g]>temp_dist[k])
                  {
                       t=temp_dist[g];
                       temp_dist[g]=temp_dist[k];
                       temp_dist[k]=t;

                       t1=idx_out[i * 8 + g]
                       idx_out[i * 8 + g]=idx_out[i * 8 + k]
                       idx_out[i * 8 + k]=t1;
                  }
             }
         }
...
__global__ void lkpo_select_four(int b, int n,float radius, const float* xyz, int* idx_out) {
    int batch_idx = blockIdx.x;
    xyz += batch_idx * n * 3;
...
void lkpoSelectLauncher(int b, int n, float radius, const float* xyz, int* idx_out) {
    lkpo_select<<<b, 512>>>(b, n, radius, xyz, idx_out);
}
void lkpoSelectFourLauncher(int b, int n, float radius, const float* xyz, int* idx_out) {
    lkpo_select_four<<<b, 512>>>(b, n, radius, xyz, idx_out);
}
