...
os.system('cp %s' % (LOG_DIR)) # bkp of model def
os.system('cp train_and_eval_scannet.py %s' % (LOG_DIR)) # bkp of train procedure
...
class SegTrainer(object):
        ...
        with tf.Graph().as_default():
            ...
            with tf.Session(config=config) as sess:
                ...
                for epoch in range(epoch_sz):
                    ...
                    log_string("epoch %d , loss is %f take %.3f s" % (epoch + 1, ave_loss, time.time() - tic))
                    tic = time.time()
                    if (epoch + 1) % 5 == 0:
                ...
                #log_string("Model saved in file: ", _path)
    ...
    def evaluate_one_epoch(self, sess, test_writer, step, epoch):
        ...
        log_string("---EVALUATE %d EPOCH---" % (epoch + 1))
        ...
        log_string('eval whole scene mean loss: %f' % (loss_sum / float(self.test_sz)))
        ...
        log_string("caliacc is %f " % caliacc)
